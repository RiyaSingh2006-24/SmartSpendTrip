...
const Signup = () => {
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    try {
      const result = await travelApi.signup({
        name: name.trim(),
        ...
        <div className="glass-card overflow-hidden">
          <div className="bg-primary px-8 py-7 text-primary-foreground">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs uppercase tracking-[0.22em] text-primary-foreground/70">Account</p>
                <h1 className="mt-2 font-heading text-3xl font-semibold">Sign up</h1>
              </div>
            </div>
            <p className="mt-4 text-sm text-primary-foreground/80">
              Create your account once, then your saved destination searches follow you automatically.
            </p>
          </div>
